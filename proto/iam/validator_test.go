...
func TestValidate_UpdateUserAdminRequest(t *testing.T) {
	cases := []struct {
		name    string
		input   *UpdateUserAdminRequest
		wantErr bool
	}{
		{
			name:    "OK",
			input:   &UpdateUserAdminRequest{UserId: 1, IsAdmin: true},
			wantErr: false,
		},
		{
			name:    "OK IsAdmin false",
			input:   &UpdateUserAdminRequest{UserId: 1, IsAdmin: false},
			wantErr: false,
		},
		{
			name:    "NG Required(UserId)",
			input:   &UpdateUserAdminRequest{UserId: 0, IsAdmin: true},
			wantErr: true,
		},
	}
	for _, c := range cases {

...
func TestShouldNotifyOrg(t *testing.T) {
	now := time.Now()
	cases := []struct {
		name                   string
		alertCondNotifications *[]model.AlertCondNotification
		existsNewFindings      bool
		want                   bool
	}{
		{
			name:                   "New findings exist - always notify",
			alertCondNotifications: &[]model.AlertCondNotification{},
			existsNewFindings:      true,
			want:                   true,
		},
		{
			name:                   "Nil notifications - notify",
			alertCondNotifications: nil,
			existsNewFindings:      false,
			want:                   true,
		},
		{
			name:                   "Empty notifications - notify",
			alertCondNotifications: &[]model.AlertCondNotification{},
			existsNewFindings:      false,
			want:                   true,
		},
		{
			name: "Recently notified within min CacheSecond - suppress",
			alertCondNotifications: &[]model.AlertCondNotification{
				{CacheSecond: 3600, NotifiedAt: now.Add(-10 * time.Minute)},
				{CacheSecond: 7200, NotifiedAt: now.Add(-30 * time.Minute)},
			},
			existsNewFindings: false,
			want:              false,
		},
		{
			name: "Notified long ago beyond min CacheSecond - notify",
			alertCondNotifications: &[]model.AlertCondNotification{
				{CacheSecond: 3600, NotifiedAt: now.Add(-2 * time.Hour)},
				{CacheSecond: 7200, NotifiedAt: now.Add(-1 * time.Hour)},
			},
			existsNewFindings: false,
			want:              true,
		},
		{
			name: "Default 30-day CacheSecond when all zero",
			alertCondNotifications: &[]model.AlertCondNotification{
				{CacheSecond: 0, NotifiedAt: now.Add(-1 * time.Hour)},
			},
			existsNewFindings: false,
			want:              false,
		},
		{
			name: "Uses minimum CacheSecond among multiple",
			alertCondNotifications: &[]model.AlertCondNotification{
				{CacheSecond: 600, NotifiedAt: now.Add(-5 * time.Minute)},
				{CacheSecond: 86400, NotifiedAt: now.Add(-1 * time.Hour)},
			},
			existsNewFindings: false,
			want:              false,
		},
		{
			name: "Uses max NotifiedAt for comparison",
			alertCondNotifications: &[]model.AlertCondNotification{
				{CacheSecond: 600, NotifiedAt: now.Add(-20 * time.Minute)},
				{CacheSecond: 600, NotifiedAt: now.Add(-5 * time.Minute)},
			},
			existsNewFindings: false,
			want:              false,
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := shouldNotifyOrg(c.alertCondNotifications, c.existsNewFindings)
			if got != c.want {

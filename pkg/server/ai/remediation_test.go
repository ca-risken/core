...
func TestGetRemediationProposal(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:  "OK",
			input: &ai.GetRemediationProposalRequest{ProjectId: 1, RemediationProposalId: 1001},
			want: &ai.GetRemediationProposalResponse{
		...
		},
		{
			...
			input:     &ai.GetRemediationProposalRequest{ProjectId: 1, RemediationProposalId: 9999},
			want:      &ai.GetRemediationProposalResponse{},
			mockError: gorm.ErrRecordNotFound,
		},
		{
			name:    "NG Validation error",
			input:   &ai.GetRemediationProposalRequest{ProjectId: 0, RemediationProposalId: 1001},
			want:    nil,
			wantErr: true,
		},
		{
	...
	}{
		{
			...
			mockResponse: []*model.RemediationProposal{
				...
				},
				{
					...
					ProjectID:             1,
					Status:                model.RemediationProposalStatusFailed,
					StatusDetail:          func() *string { s := "some error"; return &s }(),
...
func TestPutRemediationProposal(t *testing.T) {
	...
	pendingUpsertWithID := &ai.RemediationProposalForUpsert{
		RemediationProposalId: 1001,
		FindingId:             1001,
		ProjectId:             1,
		Status:                model.RemediationProposalStatusPending,
	...
	cases := []struct {
		...
		},
		{
			...
			input:      &ai.PutRemediationProposalRequest{ProjectId: 1, RemediationProposal: pendingUpsertWithID},

...
func (_m *OrganizationRepository) ListOrganization(ctx context.Context, organizationID uint32, name string, userID uint32, projectID uint32) ([]*model.Organization, error) {
	...
	var r0 []*model.Organization
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, uint32, string, uint32, uint32) ([]*model.Organization, error)); ok {
		return rf(ctx, organizationID, name, userID, projectID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, uint32, string, uint32, uint32) []*model.Organization); ok {
		r0 = rf(ctx, organizationID, name, userID, projectID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*model.Organization)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, uint32, string, uint32, uint32) error); ok {
		r1 = rf(ctx, organizationID, name, userID, projectID)
	} else {
		r1 = ret.Error(1)
	}

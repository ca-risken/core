...
func (c *Client) PutUser(ctx context.Context, u *model.User) (*model.User, error) {
	if err := c.Master.WithContext(ctx).Exec(updateUser, u.Name, convertZeroValueToNull(u.UserIdpKey), fmt.Sprintf("%t", u.Activated), fmt.Sprintf("%t", u.IsAdmin), u.UserID).Error; err != nil {
		return nil, err
	}
	return c.GetUserBySub(ctx, u.Sub)
}
...
const deleteDetachProjectRole = `delete from user_role where user_id = ? and role_id = ? and project_id = ?`

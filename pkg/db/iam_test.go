...
func TestUpsertGitHubSetting(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(regexp.QuoteMeta(insertPutUser)).WillReturnResult(sqlmock.NewResult(1, 1))
		...
		},
		{
			...
			wantErr: true,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(regexp.QuoteMeta(insertPutUser)).WillReturnError(errors.New("DB error"))

...
func TestListUserReserved(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listUserReserved + " and r.project_id = ?")).WillReturnRows(sqlmock.NewRows([]string{
		...
		},
		{
			...
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listUserReserved + " and r.project_id = ? and ur.user_idp_key = ?")).WillReturnRows(sqlmock.NewRows([]string{
					"reserved_id", "role_id", "user_idp_key", "created_at", "updated_at"}).
					AddRow(uint32(1), uint32(1), "uik1", now, now))
			},
		},

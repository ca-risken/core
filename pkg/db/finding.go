...
type FindingRepository interface {
	...
	UpsertPendFinding(ctx context.Context, findingID uint64, projectID uint32, pendUserID uint32, note string, reason string, expiredAtInt int64) (*model.PendFinding, error)
...
func (c *Client) ListFindingForOrg(
	ctx context.Context,
	organizationID, alertID uint32,
	fromScore, toScore float32,
	findingID uint64,
	...
	status finding.FindingStatus,
	sort, direction string,
	offset, limit uint32) (*[]FindingForOrg, error) {
	...
	// Add organization-specific condition
...
func (c *Client) BatchListFinding(ctx context.Context, req *finding.BatchListFindingRequest) (*[]model.Finding, error) {
	...
	join, cond, params := generateListFindingConditions(
		...
		req.FindingId, req.DataSource, req.ResourceName, req.Tag)
...
func (c *Client) ListFindingCount(
	...
	if organizationID != 0 {
		query += " inner join organization_project op on finding.project_id = op.project_id"
	}
	...
	// Add project or organization specific condition
	if organizationID != 0 {
		cond += " and op.organization_id = ?"
		params = append(params, organizationID)
	} else {
...
INSERT INTO pend_finding
  (finding_id, project_id, pend_user_id, note, reason, expired_at)
VALUES
  (?, ?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE
...
func (c *Client) UpsertPendFinding(ctx context.Context, findingID uint64, projectID uint32, pendUserID uint32, note string, reason string, expiredAtInt int64) (*model.PendFinding, error) {

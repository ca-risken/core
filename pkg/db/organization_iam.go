...
type OrganizationIAMRepository interface {
	...
	GetAdminOrganizationPolicy(ctx context.Context, userID uint32) (*[]model.OrganizationPolicy, error)
...
const getAdminOrganizationPolicy = `
select
  op.* 
from
  user u
...
func (c *Client) GetAdminOrganizationPolicy(ctx context.Context, userID uint32) (*[]model.OrganizationPolicy, error) {
	var data []model.OrganizationPolicy
	if err := c.Slave.WithContext(ctx).Raw(getAdminOrganizationPolicy, userID).Scan(&data).Error; err != nil {
		return nil, err
	}
	return &data, nil
}

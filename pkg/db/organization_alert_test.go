...
func TestListOrganizationNotificationByProjectID(t *testing.T) {
	now := time.Now()
	type args struct {
		projectID uint32
	}
	cases := []struct {
	...
	}{
		{
			...
			want: []*model.OrganizationNotification{
				{NotificationID: 1, Name: "notif1", OrganizationID: 1, Type: "slack", NotifySetting: "{}", CreatedAt: now, UpdatedAt: now},
				{NotificationID: 2, Name: "notif2", OrganizationID: 2, Type: "slack", NotifySetting: "{}", CreatedAt: now, UpdatedAt: now},
			},
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listOrganizationNotificationByProjectID)).WillReturnRows(sqlmock.NewRows([]string{
					"notification_id", "name", "organization_id", "type", "notify_setting", "created_at", "updated_at"}).
					AddRow(uint32(1), "notif1", uint32(1), "slack", "{}", now, now).
					AddRow(uint32(2), "notif2", uint32(2), "slack", "{}", now, now))
			},
		},
		{
			name:    "OK - no results",
		...
		},
		{
			...
			wantErr: true,
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(regexp.QuoteMeta(listOrganizationNotificationByProjectID)).WillReturnError(errors.New("DB error"))
			},
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			c.mockClosure(mock)
			got, err := db.ListOrganizationNotificationByProjectID(ctx, c.args.projectID)
			if err != nil && !c.wantErr {
				t.Fatalf("Unexpected error: %+v", err)
			}
			if !reflect.DeepEqual(got, c.want) {
